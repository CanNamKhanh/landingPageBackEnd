...
async function getAccessToken() {
    const res = await axios_1.default.post(`${PAYPAL_BASE}/v1/oauth2/token`, "grant_type=client_credentials", {
        auth: {
...
router.post("/create-invoice", async (req, res) => {
    ...
    try {
        ...
        // 1. Tạo draft invoice
        const invoiceRes = await axios_1.default.post(`${PAYPAL_BASE}/v2/invoicing/invoices`, {
            detail: {
        ...
        const invoiceId = invoiceRes.data.href?.split("/").pop();
        if (!invoiceId) {
        ...
        await axios_1.default.post(`${PAYPAL_BASE}/v2/invoicing/invoices/${invoiceId}/send`, { send_to_recipient: true }, { headers, timeout: 15000 });
        console.log("Invoice sent to recipient:", customerEmail);
        // 3. Lấy link thanh toán trả về FE
        const detailRes = await axios_1.default.get(`${PAYPAL_BASE}/v2/invoicing/invoices/${invoiceId}`, { headers, timeout: 10000 });
        const payLink = detailRes.data.detail?.metadata?.payer_view_url ??
            detailRes.data.links?.find((l) => l.rel === "payer-view")
                ?.href ??
            "";
        ...
        if (axios_1.default.isAxiosError(err)) {
            console.error("PayPal error:", JSON.stringify(err.response?.data ?? err.message));
...
//# sourceMappingURL=paypal.route.js.map