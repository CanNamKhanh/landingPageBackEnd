...
export async function listBoostersHandler(
  req: Request,
  res: Response,
): Promise<void> {
...
export async function updateBoosterHandler(
  req: Request,
  res: Response,
): Promise<void> {
  const body = updateBoosterRequestSchema.parse(req.body);
  const booster = await boosterService.updateBooster(
    req.params.boosterId as string,
    body,
  );
  log.info("booster updated by admin", {
    boosterId: booster.id,
    adminId: req.user!.id,
  });
